...
/**
 * @title  HashCanonNFT – Fully On-Chain Generative Mandalas
 * @author DataSattva
 ...
 * @custom:license-art CC BY-NC 4.0 + HashCanon Commercial License v1.0
 * @custom:source     https://github.com/HashCanon/nft
 */
...
contract HashCanonNFT is ERC721, ERC2981, Ownable, ReentrancyGuard {
    ...
    /*──────────────────── Events ───────────────────*/
    event HashCanonNFTDeployed(string site, string social);
    ...
    constructor(address rendererAddr)
        ERC721("HashCanon", "HCN")
        Ownable(msg.sender)
    {
        renderer = IMandalaRenderer(rendererAddr);
        _setDefaultRoyalty(payable(msg.sender), 750); // basis-points

        emit HashCanonNFTDeployed(
            "https://hashcanon.github.io/mint",
            "https://x.com/HashCanon"
        );
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        // Crown (rank:qty), calculated on the fly
        (uint8 rnk, uint8 qty) = CrownLib.crown(seed);
        string memory crown = string.concat(
            Strings.toString(rnk), ":", Strings.toString(qty)
        );
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name":"HashCanon #', id.toString(), '",',
                '"description":"HashCanon is a fully on-chain mandala: a deterministic glyph where entropy becomes form. A 256-bit cryptographic seed unfolds into self-contained SVG art, following the visual principles of the I Ching. No IPFS. No servers. Only Ethereum.",',
                '"creator":"DataSattva",',
                '"external_url":"https://hashcanon.github.io/mint",',
                '"license":"CC BY-NC 4.0 + Hash Canon Commercial License v1.0",',
    ...
    /* helper for two-digit */
    function _two(uint8 v) internal pure returns (string memory) {
