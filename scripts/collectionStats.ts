...
async function main() {
  ...
  /* ── быстрый, но безопасный майнинг ──
     майним каждые 500 tx, чтобы не выбиться из gas-limit */
  const BATCH = 500;
  for (let i = 0; i < GENESIS; i++) {
    await nft.connect(owner).mint({ value: PRICE });

    if ((i + 1) % BATCH === 0) {
      // принудительно майним блок
      await ethers.provider.send("evm_mine");
    }
  }

  console.timeEnd("mint-all");           // ← теперь действительно 8 192
  ...
  console.log(`\nTokens minted   : ${total}`);
  console.log(
    `Balanced = true : ${balanced} (${(balanced / total * 100).toFixed(2)} %)`
  );
